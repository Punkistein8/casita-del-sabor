...
<div class="flex justify-center">
  <button type="button" id="botonPagarProductos"
    ...
    data-te-toggle="modal" data-te-target="#exampleModal">Pagar Platillos</button>
</div>
    ...
    <div
      ...
      <div class="relative flex-auto p-4 bg-white" data-te-modal-body-ref>
        <form method="post" id="formularioIngresarCompra">
          ...
          <input required type="hidden" name="tipoProd_soC" id="tipoProd_soC" value="prenda-hombre">
...
<!-- Modal Agregar -->
<div data-te-modal-init
  class="fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto"
  id="modalPagarProductos" data-te-backdrop="static" data-te-keyboard="false" tabindex="-1"
...
<div class="flex flex-wrap justify-center">
  <?php foreach ($listaHombres as $prendaHombre): ?>
    ...
    <div class="flex" id="<?php echo $prendaHombre->id_ph ?>">
      <div class="flex flex-col gap-1">

        <a aria-label="Añadir al carrito" id="botonProductoID<?php echo $prendaHombre->id_ph; ?>"
        ...
        <a aria-label="Quitar del carrito" id="botonProductoIDQuitar<?php echo $prendaHombre->id_ph; ?>"
...
<script>
  $('#botonPagarProductos').click((e) => {
    $('#listaProds_soC').val(listaHombres)
  })
</script>
...
<script>
  <?php if ($listaHombres): ?>
    <?php foreach ($listaHombres as $prendaHombre): ?>
      $('#botonProductoIDQuitar<?php echo $prendaHombre->id_ph; ?>').hide()
    <?php endforeach; ?>
  <?php endif; ?>
</script>
...
<script>
  <?php if ($listaHombres): ?>
    <?php foreach ($listaHombres as $prendaHombre): ?>
      ...
      $('#botonProductoID<?php echo $prendaHombre->id_ph; ?>').click(function () {
        $('#botonProductoIDQuitar<?php echo $prendaHombre->id_ph; ?>').show()
        $('#botonProductoID<?php echo $prendaHombre->id_ph; ?>').hide()
        //agregar el id del producto a la lista
        listaHombres.push(<?php echo $prendaHombre->id_ph ?>);
        console.log(listaHombres);
        Swal.fire({
          position: 'top-end',
          title: 'Agregado al carrito',
          text: `El producto "<?php echo $prendaHombre->nombre_ph; ?>" se ha agregado al carrito`,
          icon: 'success',
      ...
      $('#botonProductoIDQuitar<?php echo $prendaHombre->id_ph; ?>').click(function () {
        $('#botonProductoID<?php echo $prendaHombre->id_ph; ?>').show()
        $('#botonProductoIDQuitar<?php echo $prendaHombre->id_ph; ?>').hide()
        //eliminar ese elemento de la lista
        listaHombres = listaHombres.filter((element) => element != <?php echo $prendaHombre->id_ph ?>)
        console.log(listaHombres);
        Swal.fire({
          position: 'top-start',
          title: 'Eliminado del carrito',
          text: `El producto "<?php echo $prendaHombre->nombre_ph; ?>" se ha eliminado del carrito`,
          icon: 'info',
          showConfirmButton: false,
          timer: 1000,
          timerProgressBar: true,
...
<script>
  ...
  $('#contenedorProductosCarrito').load('<?php echo site_url('hombres/listaCarrito') ?>')
</script>
