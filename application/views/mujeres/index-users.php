    ...
    <div
      ...
      <div class="relative flex-auto p-4 bg-white" data-te-modal-body-ref>
        <form method="post" id="formularioIngresarCompra">
          ...
          <input required type="hidden" name="tipoProd_soC" id="tipoProd_soC" value="bebidas">
...
<div class="flex flex-wrap justify-center">
  <?php foreach ($listaMujeres as $prendaMujer): ?>
    ...
    <div class="flex" id="<?php echo $prendaMujer->id_pm ?>">
      <div class="flex flex-col gap-1">

        <a aria-label="Añadir al carrito" id="botonProductoID<?php echo $prendaMujer->id_pm; ?>"
        ...
        <a aria-label="Quitar del carrito" id="botonProductoIDQuitar<?php echo $prendaMujer->id_pm; ?>"
...
<script>
  $('#botonPagarProductos').click((e) => {
    $('#listaProds_soC').val(listaMujeres)
  })
</script>
...
<script>
  <?php if ($listaMujeres): ?>
    <?php foreach ($listaMujeres as $prendaMujer): ?>
      $('#botonProductoIDQuitar<?php echo $prendaMujer->id_pm; ?>').hide()
    <?php endforeach; ?>
  <?php endif; ?>
</script>
...
<script>
  <?php if ($listaMujeres): ?>
    <?php foreach ($listaMujeres as $prendaMujer): ?>
      ...
      $('#botonProductoID<?php echo $prendaMujer->id_pm; ?>').click(function () {
        $('#botonProductoIDQuitar<?php echo $prendaMujer->id_pm; ?>').show()
        $('#botonProductoID<?php echo $prendaMujer->id_pm; ?>').hide()
        //agregar el id del producto a la lista
        listaMujeres.push(<?php echo $prendaMujer->id_pm ?>);
        console.log(listaMujeres);
        Swal.fire({
          position: 'top-end',
          title: 'Agregado al carrito',
          text: `El producto "<?php echo $prendaMujer->nombre_pm; ?>" se ha agregado al carrito`,
          icon: 'success',
      ...
      $('#botonProductoIDQuitar<?php echo $prendaMujer->id_pm; ?>').click(function () {
        $('#botonProductoID<?php echo $prendaMujer->id_pm; ?>').show()
        $('#botonProductoIDQuitar<?php echo $prendaMujer->id_pm; ?>').hide()
        //eliminar ese elemento de la lista
        listaMujeres = listaMujeres.filter((element) => element != <?php echo $prendaMujer->id_pm ?>)
        console.log(listaMujeres);
        Swal.fire({
          position: 'top-start',
          title: 'Eliminado del carrito',
          text: `El producto "<?php echo $prendaMujer->nombre_pm; ?>" se ha eliminado del carrito`,
          icon: 'info',
          showConfirmButton: false,
          timer: 1000,
          timerProgressBar: true,
...
<script>
  ...
  $('#contenedorProductosCarrito').load('<?php echo site_url('mujeres/listaCarrito') ?>')
</script>
